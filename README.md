...
Portfolio value and data on individual holdings is sent to InfluxDB (a timeseries database). This allows you to connect Grafana or another dashboard frontend and see how your portfolio is performing. There is an example dashboard used for backtesting in [./grafana](./grafana)
...
Been trying to keep it clean but it's still messy. I'm doing a greenfield serverless version instead of trying to refactor this as the code grew too large.
...
Microservices based with pretty decent separation of concerns. AMQP is used between services where things are async to improve fault tolerance. There's a REST interface for position entry/exit that is used by the edge signals code. 
...
See the [Missives](docs/missives/) directory for deep dives into some specific issues:
  ...
  - [Backtesting Architecture](docs/missives/backtesting-architecture.md)
  - [Dynamic vs Exchange Impemented Stops](docs/missives/dynamic-vs-exchange-implemented-stops.md) 

## Services

All in `/services/`. See the k8 chart for deployment.

1. `binance-orders-to-amqp`, `binance-execution-reports-to-amqp`, `execution-report-to-generic-order-update` - we run a couple of redundant instances of these and they spead accross hosts for reliability. These connect to the exchange and watch the activity on your account. They forward everything they see to an AMQP message bus for processing. Message deduplication is done by AMQP.
...
2. `edge-*` - these are the edges that watch the price action and give entry and exit signals
  ...
  1. `edge70-signals` - listens to price data and publishes events to enter and exit positions. 
3. `bert` and `telegram-send-message` - your friendly telegram bot, keeps you updated on everything the system is doing and allows you to enter and exit trades manually.
...
1. `spot-trade-abstraction` - the Trade Abstraction Service (TAS). This is for spot trades. This is the part that executes entry/exit of positions. It provides a simple abstract interface and then manages a lot internally like reducing slippage, retying, managing API rate limits, detailed execution logs, position sizing and risk management.
1. `spot-portfolio-vs-positions` - checks the holdings on the exchange match what is expected. Alerts if anything is unexpected.
...
`redis` also needs to be available. 
