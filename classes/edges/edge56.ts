...
export class Edge56 {
  ...
  private is_new_high_candle(candle: CandleChartResult | Candle) {
    return new BigNumber(candle[this.key]).isGreaterThanOrEqualTo(this.current_high_candle[this.key])
  }
  ...
  private async enter_position(candle: CandleChartResult | Candle) {
    ...
    this.send_message(
      `${this.symbol} Position entry triggered at price: ${price.toFixed()}  MCAP ${humanNumber(
  ...
  async ingest_new_candle({
    ...
    if (this.in_position) {
      ...
      if (low.isLessThan(this.lowest_price_seen_since_entry)) {
        ...
        let drawdown :BigNumber= this.percentage_change_since_entry(low)
        console.warn(`${this.symbol} drawdown is now: ${drawdown}`)
        let limit = 25
        if(drawdown.isGreaterThanOrEqualTo(limit)){
          this.send_message(`${this.symbol} hit limit drawdown trigger of ${limit}%`)
        }
      }
