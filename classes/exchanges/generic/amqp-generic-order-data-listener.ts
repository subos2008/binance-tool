...
import { GenericOrderCallbacks } from "../../../interfaces/exchanges/generic/order_callbacks"

export class AMQP_GenericOrderDataListener implements TypedMessageProcessor<GenericOrderData> {
  event_name: MyEventNameType = "GenericOrderData"
  send_message: Function
  logger: ServiceLogger
  health_and_readiness: HealthAndReadiness
  callbacks_health: HealthAndReadinessSubsystem
  order_callbacks: GenericOrderCallbacks
  ...
  async process_message(event: GenericOrderData, channel: Channel, raw_amqp_message: Message): Promise<void> {
    try {
      await this.processGenericOrderDataMessage(event)
  ...
  async processGenericOrderDataMessage(data: GenericOrderData) {
    const {
    ...
    try {
      if (this.print_all_trades) {
        ...
        this.logger.info(
          tags,
          `..totalBaseTradeQuantity: ${totalBaseTradeQuantity}, averageExecutionPrice: ${data.averageExecutionPrice}`
        )
      }
      ...
      Sentry.withScope(function (scope) {
        scope.setTag("class", "AMQP_GenericOrderDataListener")
        scope.setTag("operation", "processGenericOrderDataMessage")
