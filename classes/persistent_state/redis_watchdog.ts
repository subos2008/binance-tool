...
export class RedisWatchdog {
  ...
  constructor({
    ...
    this.logger.info(
      `Created redis watchdog for ${this.watchdog_name} expiring each ${
        this.timeout_seconds
      } seconds, redis key '${this.get_key()}'`
    )
  ...
  async reset_subsystem(subsystem_name: string): Promise<void> {
    const result = await this.set_redis_key(
      `${this.get_key()}:${subsystem_name}`,
      Date.now(),
      "EX",
      this.timeout_seconds
    )
  ...
  _reset_timer(subsystem: string) {
    ...
    this.timeout_objs[subsystem] = setTimeout(() => {
      ...
      let watchdog_name = this.watchdog_name
      Sentry.withScope(function (scope: any) {
        scope.setTag("location", "redis-watchdog-expiry")
        scope.setTag("watchdog-subsystem", subsystem)
        Sentry.captureMessage(`Watchdog timer ${watchdog_name}:${subsystem} expired!`)
      })
