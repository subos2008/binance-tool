...
export class TradeDefinition {
  ...
  constructor(logger: Logger, trade_definition: TradeDefinitionInputSpec, exchange_info?: any) {
    assert(logger)
    this.logger = logger
    ...
    this.pair = pair
    if (max_quote_amount_to_buy) this.max_quote_amount_to_buy = new BigNumber(max_quote_amount_to_buy)
    ...
    if (this.unmunged.buy_price && this.unmunged.stop_price)
      assert(this.unmunged.stop_price.isLessThan(this.unmunged.buy_price))
