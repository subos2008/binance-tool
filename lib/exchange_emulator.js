...
class ExchangeEmulator {
		...
		try {
			...
			assert(
				...
				`this.balance_not_in_orders, ${this.balance_not_in_orders(
					base_currency
				)} ${base_currency}, must be >= base_volume (${base_volume}). Balances: ${JSON.stringify(
					this.balances
				)}`
			);
			const id = this.next_order_id;
	...
	async order({ side, symbol, type, quantity, price, stopPrice } = {}) {
		...
		if (typeof stopPrice !== 'undefined') {
			...
			if (!BigNumber(stopPrice).isEqualTo(munged_price)) {
				throw new Error(
	...
	async send_ws_events(completed_orders) {
		if (!this.user_cb) return;
		let mapper = (m) => ({
			orderId: m.orderId,
			price: m.price,
			quantity: m.origQty.toFixed(),
			eventType: 'executionReport',
			symbol: m.symbol,
			orderStatus: m.orderStatus,
			orderType: m.type,
			side: m.side,
			totalTradeQuantity: m.executedQty.toFixed(),
			lastTradeQuantity: m.executedQty.toFixed()
		});
		let obj = this;
		async function callback(order) {
			assert(order);
			try {
