...
class ExchangeEmulator {
	constructor({ logger, starting_quote_balance, starting_base_balance, exchange_info } = {}) {
		...
		this.ws = { user: this.ws_user.bind(this) };
	}
	...
	async add_stop_loss_limit_sell_order({ base_volume, price, stopPrice, pair } = {}) {
		try {
			...
			this.logger.error(
				`Asserts failed in add_limit_sell_order with args: base_volume: ${base_volume}, price: ${price} stopPrice: ${stopPrice} base_coin_balance_not_in_orders: ${this
					.base_coin_balance_not_in_orders}`
			);
			throw e;
		}
