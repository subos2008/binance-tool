...
class TradeExecutor {
	...
	constructor(
		...
		if (max_quote_amount_to_buy) {
			this.max_quote_amount_to_buy = max_quote_amount_to_buy = BigNumber(max_quote_amount_to_buy);
		}
		...
		if (base_amount_held) {
			this.base_amount_held = BigNumber(base_amount_held);
		}
		...
		if (this.soft_entry) {
			...
			this.soft_entry_buy_order_trigger_price = this.buy_price.times(
				BigNumber(100).plus(percentage_before_soft_buy_price_to_add_order).div(100)
			);
			this.logger.info(`Soft entry buy order trigger price ${this.soft_entry_buy_order_trigger_price.toFixed()}`);
		}
	...
	print_percentages_for_user({ current_price } = {}) {
		try {
			...
			if (this.trading_rules) {
				this.logger.info(
					`Max portfolio loss per trade: ${this.trading_rules
						.max_allowed_portfolio_loss_percentage_per_trade}%`
				);
			}
			if (buy_price && buy_price.isZero()) {
				this.logger.info('Skipping printing percentages as buy price is unknown');
				return;
			}
	...
	shutdown_streams() {
		if (this.closeUserWebsocket || this.closeTradesWebSocket) this.logger.info(`Shutting down streams`);
	...
	async _create_stop_loss_limit_sell_order({ limit_price_factor } = { limit_price_factor: BigNumber('0.8') }) {
		...
		try {
			...
			if (!price) {
				...
				// SHIT: error placing orders for pair KNCBTC: error
				price = utils.munge_and_check_price({ exchange_info: this.exchange_info, symbol: this.pair, price });
			}
	...
	async monitor_user_stream() {
		...
		async function checkOrderFilled(data, orderFilled) {
			const { symbol, price, quantity, side, orderType, orderId, orderStatus, orderRejectReason } = data;
			...
			if (orderStatus === 'CANCELED' && orderRejectReason === 'NONE') {
				...
				obj.execution_complete(`Order was cancelled, presumably by user. Exiting.`, 1);
				return;
			}
	...
	execution_complete(msg, exit_code = 0) {
		...
		// Add call to set redis trade complete here
	...
	async placeTargetOrder() {
		try {
			...
			async_error_handler(this.logger, `error placing target sell order: ${error.body}`, error);
		}
	}
		...
		try {
			...
			if (this.buy_price) {
				if (this.buy_price.isZero()) {
					...
					if (this.soft_entry) {
						this.logger.info(`Soft entry mode: waiting for entry price before placing order`);
			...
			if ((this.stop_price && this.target_price) || this.soft_entry) {
				...
				this.closeTradesWebSocket = await this.ee.ws.aggTrades([ this.pair ], async function(trade) {
					...
					if (waiting_for_soft_entry_price) {
						if (price.isLessThanOrEqualTo(obj.soft_entry_buy_order_trigger_price)) {
							waiting_for_soft_entry_price = false;
							obj.send_message(`${symbol} soft entry buy order trigger price hit`);
			...
			async_error_handler(this.logger, `exception in main loop: ${error.body}`, error);
		}
	}
}
