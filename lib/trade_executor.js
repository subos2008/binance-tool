...
class TradeExecutor {
  ...
  constructor({
    ...
    this.percentages = percentages; // wtf is this? it means to exit after dumping to console. it has been removed
    ...
    assert(!buy_price.isZero()); // depricated way of specifying a market buy
      ...
      try {
        ...
        if (orderId === (await this.trade_state.get_buyOrderId())) {
          await checkOrderFilled(data, async () => {
            ...
            await this.trade_state.set_base_amount_held(
  ...
  async placeSellOrder() {
    if (
      ...
      console.log(
        `placeSellOrder: orders already exist, skipping and heading into main loop.`
      );
      return;
    }
  ...
  async main() {
    ...
    try {
      ...
      if (
        ...
        !(await await this.trade_state.get_base_amount_held())
      ) {
        // Cases:
