...
function _break_up_binance_pair(pair) {
	try {
...
function base_currency_for_binance_pair(pair) {
	const [ total, base_coin, quote_coin ] = _break_up_binance_pair(pair);
	assert(base_coin);
	return base_coin;
}
...
function _get_symbol_filters({ exchange_info, symbol } = {}) {
	...
	if (!this.symbolData) {
		// TODO: some kind of UnrecognisedPair error?
...
// Binance
function munge_and_check_quantity({ name, volume, exchange_info, symbol } = {}) {
	...
	if (volume.isLessThan(minQty)) {
		throw new Error(`${name} ${volume} does not meet minimum quantity (LOT_SIZE): ${minQty}.`);
	}
	return volume;
}

// Binance
function munge_and_check_price({ name, price, exchange_info, symbol } = {}) {
	assert(typeof price !== 'undefined');
	assert(exchange_info);
	assert(symbol);
	price = BigNumber(price);
	if (price.isZero()) {
		throw new Error('refactor: dont pass zero price to munging functions, handle upstream');
	...
	if (price.isLessThan(minPrice)) {
		throw new Error(`${name} ${price} does not meet minimum order price (PRICE_FILTER): ${minPrice}.`);
	}
	return price;
}

// Binance
function check_notional({ name, price, volume, exchange_info, symbol } = {}) {
	assert(typeof volume !== 'undefined');
	assert(typeof price !== 'undefined');
	assert(exchange_info);
	assert(symbol);
	...
	if (quote_volume.isLessThan(minNotional)) {
		throw new Error(
			`${name} does not meet minimum order value ${minNotional} (MIN_NOTIONAL) (Buy of ${volume} at ${price} = ${quote_volume}).`
		);
	}
}
