...
export class PortfolioUtils {
  ...
  total_balance(balance: FuturesBalance) {
    let free = new BigNumber(balance.walletBalance)
    return free.toFixed(8)
  }

  total_balance_bignum(balance: FuturesBalance) {
    return new BigNumber(balance.walletBalance)
  }
  ...
  get_total_value_of_balance_in_quote_currency({
    ...
    let base_quantity = new BigNumber(balance.walletBalance)
  ...
  get_balances_with_free_greater_than({
    ...
    let mapper = (b: FuturesBalance): Mapped => {
      let quote_amount
      try {
  ...
  add_quote_value_to_portfolio_balances({
    ...
    portfolio: FuturesPortfolio
    quote_currency: string
  }): { portfolio: FuturesPortfolio; unprocessed_balances: string[] } {
  ...
  balances_to_string(portfolio: FuturesPortfolio, quote_currency: string): string | null {
  ...
  balance_for_asset({ asset, portfolio }: { asset: string; portfolio: FuturesPortfolio }) {
