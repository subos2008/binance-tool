...
import { ADX_STRING_CANDLE, ADX_Indicator, ADX_parameters } from "../classes/indicators/adx"
...
const input_unmunged = {
  ...
  period: adx_parameters.adx_period,
}
...
describe("ADX (Average Directional Index)", function () {
  describe("Author's dataset", function () {
    ...
    describe("EntrySignals(initial_candles) only", function () {
      it("matches all the expected output values", function () {
        const foo = setup(input)
        expect(foo.adx.getResult()).to.deep.equal(expectResult)
      })
    })
    ...
    describe("no initial candles", function () {
      it("matches all the expected output values", function () {
        ...
        for (let i = 0; i < input.length; i++) {
          foo.ingest_new_candle({ candle: input[i], symbol, timeframe: "1d" })
          if (foo.current_result) result.push(foo.current_result)
        }
