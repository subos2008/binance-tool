...
describe('Algo', function() {
	...
	describe('constructor', function() {
		describe('base_amount munging', function() {
			it('when passed buy_price and base_amount it passes base_amount_to_buy', function() {
				let { algo, ee } = setup({
					algo_config: {
						buy_price: BigNumber(1),
						base_amount: BigNumber(1)
					},
					no_agitate: true
				});
				expect(algo.trade_executor).to.have.property(`base_amount_to_buy`);
				expect(algo.trade_executor).not.to.have.property(`base_amount_held`);
				expect(algo.trade_executor.base_amount_to_buy).to.bignumber.equal('1');
			});
			it('when passed base_amount without buy_price it passes base_amount_held', function() {
				let { algo, ee } = setup({
					algo_config: {
						base_amount: BigNumber(1)
					},
				...
				expect(algo.trade_executor).to.have.property(`base_amount_held`);
				expect(algo.trade_executor).not.to.have.property(`base_amount_to_buy`);
				expect(algo.trade_executor.base_amount_held).to.bignumber.equal('1');
			});
		});
	});
	...
	it.skip(
		...
		async function() {
			...
			let { ee, algo } = setup({
				...
				ee_config: { starting_base_balance: base_amount, logger }
			});
			try {
				await algo.main();
				await ee.set_current_price({ symbol: default_pair, price: target_price });
				expect(most_recent_message()).to.be.a('string');
