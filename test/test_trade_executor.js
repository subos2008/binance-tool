...
describe("TradeExecutor", function() {
  ...
  describe("when only a buy_price and a stop_price present", function() {
    describe("with soft_entry", function() {
      ...
      it(
        "doesnt error from assuming a target_price is specified (regression test"
      );
    });
    it("doesnt buy if price is below the stop_price");
    it(
      "responds appropriately if the exchange rejects the stop order because it would execute immediately"
    );
    it(
  ...
  describe("when only a stop_price present", function() {
    it(
      "WARNING: the ONLY test I had that stop losses were working is the one that checks sent_message!"
    );
    it(
      'Filters for "Filter failure: PERCENT_PRICE" so that it fails fast instead of when setting stop'
    );
    ...
    it(
      "deals with the fact that Binance rejects STOP_LOSS_LIMIT_ORDERS that would trigger immediately"
    );
    ...
    describe("when base_amount_held is supplied", function() {
      it("creates a stop order", async function() {
        ...
        expect(ee.open_orders[0].origQty.isEqualTo(base_amount_held)).to.equal(
          true
        );
      });
    });
  ...
  describe("when only a target_price present", function() {
    describe("when base_amount_held is supplied", function() {
      ...
      it(
        "sends a message when the target_price is hit (currently only notifies when order is filled)"
      );
    });
  });
  describe("when only a stop_price and a target_price present", function() {
    ...
    it(
      "switches from stop to target a percentage before the actual price is hit"
    );
    it(
      "switches from target to stop a percentage before the actual price is hit"
    );
  ...
  describe("Persistance / restartability)", function() {
    ...
    it(
      "can adopt a certain base_amount of existing base currency into a trade"
    );
  });
  ...
  it(
    "when auto-size is not specified we can buy more than the trading rules would allow"
  );
  it(
    "should autosize automatically and only ignore trading rules if --ignore-trading-rules is set"
  );

  describe("Selling (vs stop))", function() {
    it(
      "can calculate the base_amount to sell to reduce a position by a quote_amount at spot"
    );
  });
  it(
